...
overload .stamp with $D2E.d2cst_get_stamp
overload .stamp with $D2E.d2var_get_stamp
...
#staload _(*TMP*) = "{$XANADU}/DATS/dynexp2_print.dats"

(* ****** ****** *)
//
implement fprint_val<ir0val> = fprint_ir0val
//
(* ****** ****** *)
//
extern
fun the_d2cstdef_search(k0: d2cst): Option_vt(ir0val)
extern
fun the_d2cstdef_insert(d2c: d2cst, def: ir0val): void
//
extern
fun the_d2vardef_search(k0: d2var): Option_vt(ir0val)
extern
fun the_d2vardef_insert(d2v: d2var, def: ir0val): void
//
(* ****** ****** *)
//
datatype
...
local
//
absimpl ir0env_tbox = List0(@(d2key, ir0val))
//
datavtype intpenv =
...
<<<<<<< HEAD
fun intplst_make_fenv(kxs: ir0env): intplst =
  auxlst(kxs, intplst_fun())
  where
    fun auxlst(kxs: ir0env, env: intplst): intplst =
      case+ kxs of
      | list_nil() => env
      | list_cons(kx0, kxs) => (auxlst(kxs, env)) where
          val env = intplst_cons(kx0.0, kx0.1, env)
        end
  end
=======
*)
...
fun intplst_take_fenv(env: !intplst): ir0env =
  ...
  where
    vtypedef res = List0_vt(@(d2key, ir0val))
    fun auxenv(env: !intplst, res: res): res =
      case+ env of
        | intplst_nil() => res
        | intplst_fun() => res

        | intplst_let1(env) => auxenv(env, res)
        | intplst_loc1(env) => auxenv(env, res)
        | intplst_loc2(env) => auxenv(env, res)
...
//
implement intpenv_take_fenv(env) =
  intplst_take_fenv(xs)
  where
    val+INTPENV(l0, xs) = env
  end
...
implement intpenv_bind_fix(env0, irv0) =
  let
    val-IR0Vfix(fenv, d2v0, _, _) = irv0
  in
    (fold@(env0))
    where
      val+@INTPENV(l0, xs) = env0
      val () = (xs := intplst_cons(D2Kvar(d2v0), irv0, xs))
    end
  end

implement intpenv_bind_fixs(env0, irv0) = auxirdfs(env0, irdfs)
  where
    val-IR0Vfixs(fenv, d2v0, iras, body, irdfs) = irv0
    fun auxirdfs(env0: !intpenv, ires: ir0explst): void =
      case+ ires of
        | list_nil((*void*)) => ()
        | list_cons(ire1, ires) => (fold@(env0); auxirdfs(env0, ires))
          where
            val+@INTPENV(l0, xs) = env0
            val () = (xs := intplst_cons(D2Kvar(d2v1), irv1, xs))
          end
          where
            val- IR0Efix(knd1, d2v1, iras, body) = ire1.node()
            val irv1 = IR0Vfixs(fenv, d2v1, iras, body, irdfs)
          end
  end

(* ****** ****** *)

implement intpenv_pop0_let1(env0) = (fold@(env0))
  where
    fun auxlst(xs: intplst): intplst =
    (case- xs of
      | ~intplst_let1(xs) => xs
      | ~intplst_cons(_, _, xs) => auxlst(xs))
    val-@INTPENV(l0, xs) = env0
...
implement intpenv_push_let1(env0) = (fold@(env0))
  where
    val-@INTPENV(l0, xs) = env0
    val () = (xs := intplst_let1(xs))
  end

(* ****** ****** *)

implement intpenv_free_nil(env0) =
  {
    val-~intplst_nil() = xs
  }
  where
...
implement intpenv_free_fenv(env) = (auxlst(xs))
  where
    fun auxlst(xs: intplst): void =
      (case- xs of
        | ~intplst_fun() => ()
        | ~intplst_cons(_, _, xs) => auxlst(xs))
...
implement interp0_search_d2cst(env0, d2c0) = (auxlst(xs))
  where
    vtypedef res = Option_vt(ir0val)
    val+INTPENV(l0, xs) = env0
    fun auxlst(xs: !intplst): res =
      (case+ xs of
        | intplst_nil() => the_d2cstdef_search(d2c0)
        | intplst_fun() => the_d2cstdef_search(d2c0)
        | intplst_let1(xs) => auxlst(xs)
        | intplst_loc1(xs) => auxlst(xs)
        | intplst_loc2(xs) => auxlst(xs)
        | intplst_cons(d2k1, irv1, xs) =>
          (case+ d2k1 of
            | D2Kcst(d2c1) =>
              if (d2c0 = d2c1) then Some_vt(irv1)
              else auxlst(xs)
            | D2Kvar(d2v1) => auxlst(xs))
      )
  end
...
implement interp0_search_d2var(env0, d2v0) = (auxlst(xs))
  where
    vtypedef res = Option_vt(ir0val)
    val+INTPENV(l0, xs) = env0
    fun auxlst(xs: !intplst): res =
      (case+ xs of
        | intplst_nil() => the_d2vardef_search(d2v0)
        | intplst_fun() => the_d2vardef_search(d2v0)
        | intplst_let1(xs) => auxlst(xs)
        | intplst_loc1(xs) => auxlst(xs)
        | intplst_loc2(xs) => auxlst(xs)
        | intplst_cons(d2k1, irv1, xs) =>
...
implement interp0_insert_d2cst(env0, d2c0, irv0) =
  let
    val+@INTPENV(l0, xs) = env0
  in
    case xs of
      | intplst_nil() => (fold@(env0); the_d2cstdef_insert(d2c0, irv0))
      | _(*non-intplst_nil*) => (fold@(env0))
        where
          val () = (xs := intplst_cons(D2Kcst(d2c0), irv0, xs))
        end (* non-intplst_nil *)
  end

(* ****** ****** *)

implement interp0_insert_d2var(env0, d2v0, irv0) =
  let
    val+@INTPENV(l0, xs) = env0
  in
    case xs of
    | intplst_nil() =>
      (fold@(env0); the_d2vardef_insert(d2v0, irv0))
    | _(*non-intplst_nil*) =>
      (fold@(env0))
        where
          val () = (xs := intplst_cons(D2Kvar(d2v0), irv0, xs))
...
fun gint_lt_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vbtf(x < y) end

fun gint_gt_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vbtf(x > y) end

fun gint_lte_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vbtf(x <= y) end
...
fun gint_cmp_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vint(compare(x, y)) end


fun gint_add_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vint(x + y) end

fun gint_sub_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vint(x - y) end


fun gint_mul_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vint(x * y) end

fun gint_div_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vint(x / y) end

fun gint_mod_sint_sint(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vint(x) = x; val-IR0Vint(y) = y in IR0Vint(x % y) end
...
implement hash_key<key>(k0) =
  let
    fun fhash(k0: uint): ulint = hash_key<uint>(k0)
  in
    $effmask_all(fhash($STM.stamp2uint(k0.stamp())))
  end

implement equal_key_key<key>(k1, k2) =
  $effmask_all($STM.eq_stamp_stamp(k1.stamp(), k2.stamp()))

val the_d2cstdef_map =
  hashtbl_make_nil<key,itm>(i2sz(D2CSTMAPSZ))

in (*in-of-local*)

implement interp0_fprint_d2cstmap(out) =
  (fprint_hashtbl(out, the_d2cstdef_map))


implement the_d2cstdef_search(k0) =
  hashtbl_search<key,itm>(the_d2cstdef_map, k0)

implement the_d2cstdef_insert(k0, x0) =
{
  (*
  val () = println!("the_d2cstdef_insert: k0 = ", k0)
  val () = println!("the_d2cstdef_insert: x0 = ", x0)
  *)
  val-~None_vt() =
    hashtbl_insert<key,itm>(the_d2cstdef_map, k0, x0)
}
...
implement interp0_fprint_d2varmap(out) =
  fprint_hashtbl(out, the_d2vardef_map)


implement the_d2vardef_search(k0) =
  hashtbl_search<key,itm>(the_d2vardef_map, k0)

implement the_d2vardef_insert(k0, x0) =
{
  val-~None_vt() = hashtbl_insert<key,itm>(the_d2vardef_map, k0, x0)
}
...
fun string_append(x: ir0val, y: ir0val): ir0val =
  let val-IR0Vstr(x0) = x; val-IR0Vstr(y0) = y
  in
    IR0Vstr(strptr2string(string0_append(x0, y0)))
  end

fun int2str(x: ir0val): ir0val =
  let
    val-IR0Vint(x0) = x
    val str = tostring_int(x0)
    in
    IR0Vstr(str)
  end

fun tostring(x0: ir0val): ir0val =
  IR0Vstr(res)
  where
    val res =
      (case- x0 of
        | IR0Vint(x) => tostring_int(x)
        | IR0Vchr(x) => tostring_char(x)
        | IR0Vbtf(x) => tostring_bool(x)
        | IR0Vflt(x) => tostring_double(x)
      )
  end
...
fun firfun1(f1: (ir0val) -> ir0val): ir0valist -<cloref1> ir0val =
  lam(vs) =>
    let
      val-list_cons(v1, vs) = vs
    in
...
fun firfun2(f2: (ir0val, ir0val) -> ir0val): ir0valist -<cloref1> ir0val =
  lam(vs) =>
    let
      val-list_cons(v1, vs) = vs
      val-list_cons(v2, vs) = vs
...
fun d2cst(nam: string): d2cst =
  (let
    //
    (*
    val () = println!("d2cst: nam = ", nam)
    *)
    //
    val sym = $SYM.symbol_make(nam)
    val opt = $T12.the_dexpenv_find(sym)
  in
    case- opt of
      | ~Some_vt(d2i) =>
        (case- d2i of
...
implement interp0_initize_gint() =
  ...
  in
    ...
    then
    {
      val () = the_d2cstdef_insert
      (
        d2cst("tostring"), IR0Vfun(firfun1(tostring))
      )

(*
      val () = the_d2cstdef_insert
      (
        d2cst("int2str"), IR0Vfun(firfun1(int2str))
      )
*)

      val () = the_d2cstdef_insert
      (
        d2cst("string_append"), IR0Vfun(firfun2(string_append))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_lt_sint_sint"), IR0Vfun(firfun2(gint_lt_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_gt_sint_sint"), IR0Vfun(firfun2(gint_gt_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_eq_sint_sint"), IR0Vfun(firfun2(gint_eq_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_lte_sint_sint"), IR0Vfun(firfun2(gint_lte_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_gte_sint_sint"), IR0Vfun(firfun2(gint_gte_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_neq_sint_sint"), IR0Vfun(firfun2(gint_neq_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_cmp_sint_sint"), IR0Vfun(firfun2(gint_cmp_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_add_sint_sint"), IR0Vfun(firfun2(gint_add_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_sub_sint_sint"), IR0Vfun(firfun2(gint_sub_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_mul_sint_sint"), IR0Vfun(firfun2(gint_mul_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_div_sint_sint"), IR0Vfun(firfun2(gint_div_sint_sint))
      )
      val () = the_d2cstdef_insert
      (
        d2cst("gint_mod_sint_sint"), IR0Vfun(firfun2(gint_mod_sint_sint))
      )
  }
  end (* end of [interp0_initize_gint] *)
=======
